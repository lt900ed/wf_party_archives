$(document).ready(function () {
  ...
  const assetPath = '/assets/root/img/';
  var socket = io();
  var charLoaded = false;
  var equipLoaded = false;
  var waitingForUrl = false;
  var blank_elem = $('<li class="unit"><img src="/assets/root/img/square_0_244.png"></li>');
if (window.matchMedia && window.matchMedia('(prefers-color-scheme: dark)').matches) {
  ...
  socket.on('url added', function (url) {
    const shareUrl = "http://ecrire-bot.herokuapp.com/" + url.id
  ...
  $("#btnSave").on("click", function () {
	...
	window.history.pushState("saved", "", "http://ecrire-bot.herokuapp.com");
    $(this).removeClass("on");
    setTimeout(function () {
      $("#btnSave").addClass("on")
    }, 100);
  ...
  $("#btnGetCompURL").on("click", function () {
    ...
    const imageUrl = "http://ecrire-bot.herokuapp.com/comp/" + units.join('-') +lngcode+ ".png";
